...
def main(custom_commandline=None):
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        print('IS_STUB1: ', esp.IS_STUB)
        read_mac(esp, args)

        print('IS_STUB15: ', esp.IS_STUB)
        if not args.no_stub:
            esp = esp.run_stub()

        print('IS_STUB2: ', esp.IS_STUB)
        print('override vddsdio: ', args.override_vddsdio)
        if args.override_vddsdio:
            esp.override_vddsdio(args.override_vddsdio)

        print('IS_STUB3: ', esp.IS_STUB)
        if args.baud > initial_baud:
            try:
                print('change baud to ', args.baud)
                esp.change_baud(args.baud)
            except NotImplementedInROMError:
                print("WARNING: ROM doesn't support changing baud rate. Keeping initial baud rate %d" % initial_baud)

        print('IS_STUB4: ', esp.IS_STUB)
